...
- 검증: `npm.cmd run typecheck`, `npm.cmd run build` 통과. 기존 이미지 최적화 경고만 남아 있다.
## [2026-07-22] 테무·쿠팡 상품정보 자동입력 보정

- 테무 페이지에서는 일반 스크립트 안의 첫 번째 `price`·`image` 값을 상품 정보로 오인하지 않도록 Product 구조화 데이터와 상품 메타데이터만 사용하게 제한했다.
- 쿠팡 파트너스 iframe 태그를 상품 링크 입력란에 그대로 붙여넣을 수 있게 하고, 리다이렉트 주소에 포함된 상품명·상품 이미지·실제 제휴 구매 링크를 추출하도록 했다.
- 쿠팡 iframe 응답에는 가격 정보가 없으므로 임의 가격을 넣지 않고 직접 입력 안내를 표시하며, 일반 쿠팡 링크가 외부 조회를 차단하면 iframe 사용 안내를 표시한다.
- 상품 등록 폼 상단에 작은 `입력 초기화` 버튼을 추가해 링크·상품명·가격·이미지·설명을 한 번에 비울 수 있게 했다.
- 검증: 실제 제공된 `coupa.ng/cofghH` 리다이렉트 응답 구조 확인, `npm.cmd run typecheck`, `npm.cmd run build`, `git diff --check` 통과.
## [2026-07-22] 학생 랭킹 기간 표시 위치 정리

- 학생 홈의 `랭킹 TOP 5`에서 `커스텀` 같은 랭킹 내부 단위 명칭을 제거했다.
- `수능 특강`, `전체` 등 반 선택 항목과 실제 집계 기간을 같은 줄에 배치하고, 기간은 오른쪽 정렬과 반 선택 항목에 맞춘 글꼴 크기·굵기로 표시했다.
- 좁은 화면에서는 반 선택 영역만 가로 스크롤되고 기간은 오른쪽에 고정되도록 구성했다.
- 검증: `npm.cmd run typecheck`, `npm.cmd run build`, `git diff --check` 통과.
## [2026-07-22] 랭킹 집계 한국 날짜 기준 보정

- 랭킹 기간 계산이 한국 시간의 날짜를 UTC 날짜 문자열로 변환하면서 하루 전으로 밀리던 문제를 수정했다.
- 일간·주간·월간·분기·최근 N일 기간 계산을 모두 한국 날짜를 기준으로 계산하도록 통일했다.
- 스티커 원장의 `created_at`도 UTC 문자열 앞 10자리가 아니라 한국 날짜로 변환한 뒤 랭킹 기간과 비교하도록 변경했다.
- 전체 랭킹은 기간 안의 모든 반 원장을 합산하고, 그룹 랭킹은 해당 그룹으로 지급된 원장만 합산하는 기존 범위를 확인했다.
- 검증: 한국 시간 `2026-07-22` 날짜 변환 확인, `npm.cmd run typecheck`, `npm.cmd run build`, `git diff --check` 통과.
## [2026-07-22] 이벤트 진입·경품 선호·학생 생일·관리자 이벤트 UX 개편

- 학생 홈 이벤트 블록의 `다른 이벤트` 버튼을 없애고 `전체 이벤트`, `이벤트 경품` 버튼을 직접 배치해 각 화면을 바로 열도록 했다.
- 학생 경품 선호 표현을 `좋아요`에서 `받고 싶다`로 변경하고 선택 상태는 `받고 싶어요`로 표시했다.
- 학생 가입의 나이 입력을 생년월일 입력으로 교체하고, 학생 마이페이지에서 이름 옆에 `M월 D일` 생일을 표시·수정할 수 있게 했다.
- 관리자 학생 목록의 나이 열을 생년월일로 교체하고 오늘 생일인 학생에게 `오늘 생일` 표시를 추가했다.
- 관리자 경품 리스트에 받고 싶다고 선택한 학생 이름과 생일을 상품별로 표시해 생일 선물 선호를 확인할 수 있게 했다.
- 관리자 이벤트/상품 관리를 `이벤트 리스트`, `경품 리스트 관리` 상단 탭으로 나누고 이벤트 리스트 하단의 `＋ 이벤트 생성` 버튼에서 생성 팝업을 열도록 변경했다.
- `students.birth_date`를 추가하는 `20260722_22_student_birth_date.sql` 마이그레이션을 추가했다. 기존 학생은 마이페이지에서 생년월일을 입력하면 연동된다.
- 검증: `npm.cmd run typecheck`, `npm.cmd run build`, `git diff --check` 통과. 기존 이미지 최적화 경고만 남아 있다.
## [2026-07-22] 쿠팡 iframe 조회 차단 시 상품 등록 허용

- 쿠팡 iframe 태그를 붙여넣으면 클라이언트에서 `src` 주소를 즉시 추출해 저장 가능한 구매 링크로 변환하도록 했다.
- Vercel 서버의 쿠팡 자동조회가 차단되어도 기존에 입력한 상품명·가격·이미지를 유지하고 상품 등록을 계속할 수 있게 했다.
- 상품 등록 시에도 iframe 원문이 남아 있으면 `src`를 다시 정규화해 구매 링크 유효성 검사와 저장에 사용하도록 했다.
- iframe 조회 실패 안내를 재입력을 요구하는 문구에서 `상품 정보를 확인한 뒤 등록 가능` 안내로 변경했다.
- 검증: `npm.cmd run typecheck`, `npm.cmd run build`, `git diff --check` 통과.
## [2026-07-22] 공지 표시 및 학생 홈 이벤트 인라인 탭 개선

- 공지 본문에 `whitespace-pre-wrap`과 단어 줄바꿈을 적용해 관리자가 입력한 개행을 학생 공지와 관리자 목록에서 그대로 표시하도록 했다.
- 공지 첨부 이미지와 관리자 미리보기를 1:1 정방형 영역에 `object-cover`로 가득 채워 표시하도록 변경했다.
- 학생 홈 공지 배너의 최소 높이와 제목 줄 높이를 늘려 공지 제목이 위아래로 잘리지 않게 했다.
- 학생 홈 이벤트 블록의 전체화면 팝업을 제거하고 `전체 이벤트`, `이벤트 경품`을 블록 내부 탭으로 구현했다.
- 전체 이벤트 탭은 블록 안에서 이벤트 아코디언을, 이벤트 경품 탭은 블록 안에서 경품 선호 카드를 직접 표시한다.
- 검증: `npm.cmd run typecheck`, `npm.cmd run build`, `git diff --check` 통과. 기존 이미지 최적화 경고만 남아 있다.
## [2026-07-22] 학생 이벤트 경품 깨진 이미지 대체 처리

- 실제 파일이 없던 `/images/placeholder-product.svg`를 선물 상자 형태의 정방형 기본 경품 이미지로 추가했다.
- 학생 이벤트 경품 탭에서 외부 이미지가 만료되거나 직접 표시가 차단되면 기본 경품 이미지로 자동 교체하도록 했다.
- 전체 이벤트 아코디언의 경품 카드에도 같은 이미지 실패 대체 처리를 적용했다.
- 검증: `npm.cmd run typecheck`, `npm.cmd run build`, `git diff --check` 통과.
## [2026-07-22] 개발자 상품 연동·선호 경품·학생 이벤트 목록 개선

- 개발자 상품을 수정하면 해당 원본을 담은 모든 관리자 경품 리스트의 상품명, 가격, 카테고리, 이미지, 구매 링크, 설명이 함께 갱신되도록 했다.
- 관리자 경품 리스트에서 개발자 연동 상품은 수정 버튼을 숨기고 API에서도 수정을 차단했다. 관리자가 직접 등록한 상품만 수정할 수 있으며 구매와 삭제는 유지했다.
- 상품 카드의 `받고 싶은 학생` 이름 목록을 제거하고, 학생 관리에서 학생 이름을 누르면 해당 학생이 받고 싶어 한 선물을 아코디언으로 확인하도록 옮겼다.
- 학생 홈 전체 이벤트는 소속 그룹 이벤트를 우선 표시하고 첫 이벤트를 기본 펼침 처리했다. 목록은 페이지당 최대 3개이며 다른 그룹 이벤트는 별도 더보기로 분리했다.
- 이벤트 경품은 모바일 3열, 페이지당 6개로 축소하고 카드/목록 보기 전환과 페이지 이동을 추가했다.
- 검증: `npm.cmd run build` 통과. 기존 이미지 최적화 경고와 빌드 캐시 경고만 남아 있다.
## [2026-07-22] 모바일 이벤트 상품 표 간격 축소

- 이벤트 목록의 순위/상품명 표를 고정 레이아웃으로 변경하고 모바일 순위 열을 내용 너비에 가깝게 축소했다.
- 모바일 셀 좌우 패딩을 줄여 순위와 상품 사이의 불필요한 공백을 제거하고 상품명이 사용할 수 있는 공간을 늘렸다.
## [2026-07-22] 상품 설명 줄바꿈 표시 개선

- 개발자 상품 리스트와 관리자 경품 리스트의 `더보기` 설명에 입력된 줄바꿈이 화면에서도 유지되도록 변경했다.
- 긴 링크나 공백 없는 문자열도 카드 너비를 벗어나지 않고 자동 줄바꿈되도록 보완했다.
## [2026-07-22] 직접 등록 상품 이미지 저장 방식 개선

- 상품 이미지 선택 시 브라우저의 Base64 임시 문자열을 DB에 직접 저장하던 방식을 제거하고 Supabase Storage의 공개 파일 URL을 저장하도록 변경했다.
- 개발자 상품의 구매 이미지·경품 이미지와 관리자 직접 등록 경품 이미지에 동일한 업로드 API를 적용했다.
- JPG, PNG, WEBP, GIF 형식과 8MB 제한을 검증하며 업로드 진행 상태와 실패 메시지를 표시하도록 했다.
## [2026-07-22] 학생 이벤트의 비소속 그룹 이벤트 제거

- 학생 홈 이벤트 블록에서 `다른 이벤트 둘러보기`, `다른 이벤트 더보기` 문구와 동작을 제거했다.
- 전체 이벤트에는 전체 대상 이벤트와 학생이 실제 소속된 반의 이벤트만 표시하며, 3개 초과 시 소속 이벤트 내부에서만 페이지 이동하도록 유지했다.
## [2026-07-22] 추천상품 카드 동작 및 랭킹 종료 후 경품 선택 흐름 구현

- 관리자 추천상품에서 별도 구매하기 버튼을 제거하고 상품 이미지를 누르면 구매 페이지가 열리도록 변경했다. 추천상품에는 `경품 리스트 담기`만 남기고 이미 담은 상품은 추천 목록에서 숨겼다.
- 경품 리스트에서도 상품 이미지를 누르면 구매 페이지가 열리며, TEMU가 포함된 링크의 제휴사 표기를 `TEMU`로 통일했다.
- 추천상품을 경품 리스트에 담을 때 외부/Base64 이미지를 Storage에 복제해 안정적인 URL로 저장하고, 기존 경품 이미지도 관리자 목록 조회 시 같은 방식으로 복구하도록 했다.
- 이벤트 종료일은 한국시간 기준 해당 날짜 23:59:59까지 활성 상태이고 다음 날 00:00부터 마감되도록 통일했다.
- 당첨 학생에게 축하·콘페티 팝업과 알림을 표시하고 `선물 고르러 가기`로 마이페이지 경품 선택 영역에 연결했다.
- 마이페이지에서 당첨 이벤트별 순위와 선택 차례를 확인하고, 1등부터 순차적으로 남은 경품을 선택하도록 했다. 선택 완료한 경품은 이벤트명·순위·선택일과 함께 `내가 받은 경품`에 표시한다.
- 학생 홈 데이터에 전체 승인 소속 정보를 포함해 그룹 이벤트의 실제 전체 학생 순위로 당첨 여부를 계산하도록 수정했다.
## [2026-07-22] 추천상품→경품 리스트 이미지 누락 재수정

- 빈 문자열인 경품 이미지가 구매 상품 이미지보다 우선 복사되던 문제를 수정하고, 상품 저장 시 빈 이미지 주소를 모두 `null`로 정규화했다.
- 경품 리스트 조회 시 연결된 추천상품 원본의 경품 이미지와 구매 이미지를 함께 대조해 올바른 값을 선택하도록 복구 로직을 강화했다.
- 관리자 조회뿐 아니라 학생 이벤트 경품 조회에서도 같은 복구를 실행해 관리자 화면을 먼저 열지 않아도 학생 화면에 정상 이미지가 전달되도록 했다.
- 기존 `marketplace_products`, `product_catalog`, `reward_items`의 빈 이미지 값을 일괄 복구하는 마이그레이션을 추가했다.
## [2026-07-22] 학생 랭킹 TOP5 기간 텍스트 스타일 조정

- 랭킹 TOP5에서 그룹명 오른쪽에 표시되는 기간을 굵은 흰색 글씨에서 얇은 회색 글씨로 변경해 시각적 우선순위를 낮췄다.
## [2026-07-22] 특강반 이름 수정 기능 추가

- 관리자 반 관리의 특강반 목록에서 반 이름을 바로 수정하고 저장·취소할 수 있도록 했다.
- Enter 저장과 Escape 취소를 지원하며 기본반 이름은 기존대로 수정 대상에서 제외했다.
- 이름 변경은 서버에 저장되어 학생 소속반, 랭킹, 이벤트 등 해당 반을 참조하는 모든 화면에 동일하게 반영된다.
## [2026-07-22] 모바일 출석 시간·전체 기간 랭킹·경품 원본 동기화

- 모바일 반 관리 표의 정규 출석 시간 입력 폭과 열 폭을 줄여 특강 기간 정보와 겹치거나 블록 밖으로 밀려나는 현상을 개선했다.
- 전체 랭킹 기본 단위에 `전체 기간`을 추가하고, 처음부터 현재 날짜까지의 스티커를 누적 집계하도록 계산 및 DB 제약 조건을 확장했다. 특강반 단위 선택에는 전체 기간이 노출되지 않도록 범위를 분리했다.
- 개발자 상품 수정 시 연결된 모든 관리자 경품 상품과 기존 이벤트 경품의 상품명·경품 이미지·구매 링크가 함께 갱신되도록 동기화했다.
- 관리자 및 학생 데이터 조회 시 이벤트 경품 복사본보다 연결된 상품 원본을 우선 적용해, 기존 이벤트에서도 최신 제목과 이미지가 즉시 표시되도록 보완했다.
- 이벤트 수정 시에도 연결된 상품 원본을 다시 읽어 경품 정보를 갱신하도록 처리했다.
- 상태: `npm.cmd run build` 성공. 기존 `<img>` 최적화 경고 2건만 남아 있으며 이번 변경과 무관하다.
- 다음 할 일: 배포 환경에 `20260722_24_ranking_all_time.sql` 마이그레이션을 적용한 뒤 전체 기간 랭킹 저장과 기존 이벤트 경품 동기화를 실제 데이터로 확인한다.
## [2026-07-22] 관리자 랭킹 필터 중복 제거 및 반별 집계 표시 정리

- 관리자 대시보드 랭킹의 `전체 기간`과 `전체 랭킹` 버튼을 하나의 `전체 랭킹` 버튼으로 통합했다. 전체 랭킹 설정이 전체 기간이면 설명에 `전체 기간`만 표시한다.
- 기본반·특강반 랭킹을 선택하면 해당 반에 승인된 학생만 대상으로 하고, 선택 기간 안에서 해당 반에 지급된 스티커 기록만 합산하도록 기존 계산 범위를 확인했다.
- 반별 랭킹 표의 소속 반 열에는 학생의 모든 소속을 나열하지 않고 현재 선택한 반만 표시하도록 수정했다.
- 상태: 구현 완료, TypeScript 타입 검사와 변경 파일 형식 검사를 통과했다.
## [2026-07-22] 관리자 스티커 롤백 영구 저장 및 학생 화면 갱신

- 관리자 스티커 로그에서 실행한 롤백을 화면 상태에만 반영하지 않고 `sticker_ledger`의 상태·취소 사유·취소 시각에 저장하도록 연결했다.
- 학생이 스티커 탭에 들어오거나 앱으로 다시 돌아오면 최신 서버 데이터를 다시 불러와 롤백된 기록을 전체·일별 스티커 합계에서 즉시 제외하도록 했다.
- 롤백된 기록은 이력에는 `취소`로 남지만 합계와 랭킹 계산에는 포함되지 않는다.
- 상태: 구현 완료, TypeScript 타입 검사와 변경 파일 형식 검사를 통과했다.
## [2026-07-22] 정규반·일일 스티커 기준 및 랭킹 이벤트 단순화

- 사용자 화면의 `기본반` 표현을 `정규반`으로 변경하고 기존 정규반 데이터와 신규 학원 자동 생성 반 이름을 함께 바꾸는 마이그레이션을 추가했다.
- 관리자 반 관리와 스티커 정책에서 출석 시간·시간대별 출석 설정을 제거했다. 출석은 반과 관계없이 학생당 하루 한 번만 정규반 원장에 기록한다.
- 칭찬 요청도 반 선택을 제거하고 학생당 하루 한 번만 요청하도록 서버 중복 검사를 추가했다.
- 과제는 승인된 특강반별로 하루 한 번씩 제출할 수 있어 여러 특강반의 과제 스티커를 각각 받을 수 있도록 유지했다.
- 반별 랭킹은 공통 출석·칭찬 점수와 선택한 특강반의 과제 점수만 합산한다. 정규반 랭킹에는 공통 출석·칭찬 점수만 반영한다.
- 학생 이벤트 화면에서 경품 탭·받고 싶다·축하 팝업·경품 선택·마이페이지 경품 영역을 제거하고 이벤트 리스트와 종료 이벤트 최종 순위만 남겼다.
- 관리자 이벤트 생성에서 순위별 경품 등록을 제거하고, 지난 이벤트 블록을 펼치면 학생별 최종 순위와 스티커 수를 볼 수 있도록 변경했다.
- 이벤트 수정 옆에 삭제 버튼을 추가하고 서버의 이벤트 삭제 처리까지 연결했다.
- 상태: TypeScript 검사와 프로덕션 빌드 성공. 기존 이미지 최적화 경고 2건만 남아 있다.
- 다음 할 일: 배포 환경에 `20260722_25_regular_class_and_daily_attendance.sql`을 적용하고 출석·칭찬 일일 제한 및 기존 정규반 이름 변경을 확인한다.
## [2026-07-23] 학원 소유자 선생님 삭제 외래키 처리

- `tenants.owner_teacher_id`가 참조 중인 선생님을 삭제할 때 외래키 오류가 발생하지 않도록 `tenants_owner_teacher_id_fkey`에 `ON DELETE SET NULL` 동작을 추가했다.
- 기존 운영 DB 적용용 마이그레이션과 신규 설치용 초기 스키마를 함께 수정했다.
- 선생님 계정이 삭제되면 학원 데이터는 유지되고 `owner_teacher_id`만 `null`로 변경된다.
- 상태: SQL 변경 완료, 변경 파일 형식 검사를 통과했다.
## [2026-07-23] 출석 정책 변경 후 전체·특강반 스티커 집계 보정

- 전체 랭킹은 `하루 1회 출석 + 모든 특강반 과제 + 모든 칭찬`을 합산하도록 확인했다.
- 정규반 랭킹은 `하루 1회 출석 + 모든 칭찬`, 특강반 랭킹은 `하루 1회 출석 + 해당 특강반 과제 + 모든 칭찬`을 합산하도록 공식을 통일했다.
- 과거 반별 출석 기록이 같은 날짜에 여러 건 남아 있어도 학생별 한국 날짜 기준 첫 활성 출석 한 건만 집계하는 공통 보정 함수를 추가했다.
- 학생 스티커 총합·일별 상세·랭킹·관리자 학생 목록의 총 스티커 수에 동일한 보정 공식을 적용했다.
- 기존 DB의 중복 출석 원장은 별도 마이그레이션에서 첫 건을 제외하고 롤백 처리해 원본 데이터도 새 정책과 일치시킨다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 2건만 남아 있다.

## [2026-07-26] 관리자 공지사항 목록 아코디언 전환

- 공지사항 게시판의 본문을 기본 접힘 상태로 변경하고 제목을 누르면 본문이 펼쳐지는 아코디언 목록으로 개편했다.
- 접힌 상태에서는 제목, 등록일, 고정 여부, 이미지 첨부 여부만 표시한다.
- 펼친 상태에서는 줄바꿈을 유지한 본문과 정방형 공지 이미지를 표시하고 상단 고정 설정을 바로 변경할 수 있게 했다.
- 수정과 삭제는 제목 옆의 별도 버튼으로 유지해 제목 클릭 동작과 분리했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.

## [2026-07-26] 관리자 프로필 사진 실제 저장 및 학원 정보 표시

- 관리자 프로필 사진을 Data URL 미리보기 문자열로 저장하던 방식을 제거하고 Supabase Storage의 `profile-images` 버킷에 실제 파일로 업로드하도록 변경했다.
- 업로드된 공개 이미지 URL과 관리자 이름을 프로필 API에서 DB에 저장하고 실제 갱신 행을 확인한 뒤 성공을 표시하도록 했다.
- 새로고침 직후 관리자 데이터가 늦게 로드되어도 설정 폼의 이름과 사진이 최신 DB 값으로 동기화되도록 했다.
- 관리자 상단 바에 저장된 프로필 사진을 이름과 함께 표시하도록 연결했다.
- 설정의 임시 미리보기 안내와 이메일을 제거하고 역할 및 온보딩에서 등록한 학원 이름을 표시하도록 변경했다.
- 상태: TypeScript 검사와 변경 파일 형식 검사를 통과했고 프로덕션 빌드도 성공했다. 기존 이미지 최적화 경고 3건만 남아 있다.

## [2026-07-26] 랭킹 안내 문구 및 학생 목록 열 배치 개선

- 랭킹 기본 노출 안내를 `특강반이 있으면 가장 먼저 등록한 특강반의 랭킹이 먼저 보여요. 특강반이 없으면 전체 랭킹이 보여요.`로 짧게 변경했다.
- 학생 목록에서 연결 상태 열을 제거하고 연결중 표시, 승인·해지, 연결 해지 동작을 처리 열로 통합했다.
- `총 스티커`를 `스티커`로 줄이고 해당 열 폭을 축소해 소속 반 열에 더 많은 공간을 배정했다.
- 소속 반 이름 배지를 한 줄로 유지하고 공간이 부족한 경우 해당 칸 안에서 가로로 확인할 수 있게 했다.
- 이름 앞 펼침 화살표를 제거했으며 이름 클릭으로 선호 경품 상세를 여는 기존 기능은 유지했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.

## [2026-07-26] 학생 공지 팝업 제목별 아코디언 적용

- 학생 홈의 공지 배너를 누르면 전체 공지를 제목별 접힌 블록으로 표시하도록 변경했다.
- 각 제목을 누르면 해당 공지의 정방형 이미지와 줄바꿈이 유지된 본문만 펼쳐진다.
- 공지 팝업을 닫을 때 펼친 공지 상태를 초기화하고, 고정 공지 우선 및 최신순 정렬은 유지했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.

## [2026-07-26] 공지 등록 직후 목록 깜빡임 방지

- 서버에 저장 중인 변경 작업 수와 변경 버전을 앱 상태 저장소에서 추적하도록 추가했다.
- 공지 등 변경사항을 저장하는 동안에는 주기적인 서버 동기화를 건너뛰고, 동기화 요청 도중 새 변경이 발생한 경우 해당 이전 응답으로 화면 상태를 덮어쓰지 않도록 했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.

## [2026-07-26] 이벤트 설정 정보 노출 및 즉시 수정 반영

- 관리자 이벤트 목록에서 이벤트 블록을 누르면 이벤트명, 설명, 대상 반, 순위 기준, 이벤트 기간을 읽기 전용 설정 정보로 펼쳐 보이도록 변경했다.
- 수정 저장 시 서버 응답을 기다리지 않고 화면 상태와 목록에 즉시 새 값을 반영하도록 변경했다.
- 서버 저장은 백그라운드에서 진행하며 실패한 경우 기존 설정값으로 자동 복구하고 실패 사유를 안내한다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 2건만 남아 있다.

## [2026-07-26] 출석·과제 관리자 승인 지급 및 로그인 네트워크 오류 처리

- 출석과 과제 요청을 `pending` 상태로 저장하고 요청 시에는 스티커 원장을 생성하지 않도록 변경했다.
- 관리자 승인함에 출석·과제·칭찬 대기 요청을 모두 표시하고, 승인할 때만 스티커 원장을 생성하며 반려 시 지급하지 않도록 연결했다.
- 학생 출석·과제 화면은 요청 후 `승인 요청 중`, 승인 후 `지급 완료`, 반려 후 `반려됨` 상태를 표시한다.
- 앱 상태를 15초마다 안전하게 갱신해 관리자의 승인 결과가 학생 화면에 자동 반영되도록 했다.
- 앱 상태 동기화의 네트워크 실패를 예외 처리해 로그인 중 `Failed to fetch`가 전체 런타임 오류 화면으로 번지지 않게 했다.
- 로그인 폼 하단에는 서버 연결 실패 원인을 인터넷 연결 또는 서버 상태 확인 안내로 표시한다.
- 기존 출석 기록은 승인 완료로 보존하고 신규 출석 요청부터 승인 대기 상태를 사용하는 마이그레이션을 추가했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 2건만 남아 있다.

## [2026-07-26] 관리자 설정 프로필 삭제 기능 추가

- 관리자 설정의 로그아웃 아래에 작은 텍스트 형태의 `프로필 삭제하기` 버튼을 추가했다.
- 삭제 전 학원과 학생 데이터는 유지되고 해당 관리자 계정으로 다시 로그인할 수 없다는 확인창을 표시한다.
- 현재 로그인한 선생님 본인만 인증 계정을 삭제할 수 있는 관리자 프로필 삭제 API를 추가했다.
- 선생님 삭제 시 학원·학생·공지·승인·스티커 이력을 보존하도록 관련 외래키를 `ON DELETE SET NULL`로 보강했으며, 발급자가 삭제되는 초대 링크가 학생 가입 기록을 막지 않도록 초대 링크 참조도 정리했다.
- 삭제 성공 후 인증 세션을 종료하고 첫 로그인 화면으로 이동하도록 연결했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 2건만 남아 있다.

## [2026-07-26] 출석·과제 지급 구간 적용 및 이벤트 수정 저장 보강

- 과제 완료율 지급 기준을 `90% 이상 5점`, `70~89% 4점`, `50~69% 3점`, `30~49% 2점`, `10~29% 1점`으로 통일했다.
- 출석 지급 기준을 `정시 이전 5점`, `10분 이내 지각 4점`, `30분 이내 지각 3점`, `1시간 이내 지각 2점`으로 통일하고 학생 출석 체크 화면에서 구간을 선택하도록 연결했다.
- 관리자 스티커 정책 화면에는 서버의 실제 지급 기준과 같은 두 정책표를 표시하도록 변경했다.
- 진행 중 이벤트 수정은 서버 저장 성공을 기다린 뒤 화면 상태를 갱신하고, 실패 시 오류를 표시하며 편집창을 유지하도록 변경했다.
- 이벤트 수정 API가 실제 대상 행을 갱신했는지 확인하고 대상이 없으면 404를 반환하도록 보강했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 2건만 남아 있다.

## [2026-07-26] 학생 설정 프로필 삭제 기능 추가

- 학생 설정의 로그아웃 버튼 아래에 작은 텍스트 형태의 `프로필 삭제하기` 버튼을 추가했다.
- 삭제 전 스티커·반 소속·활동 기록을 복구할 수 없다는 확인창을 표시한다.
- 현재 로그인한 학생 본인만 자신의 인증 계정을 삭제할 수 있는 서버 API를 추가했으며, 학생을 참조하는 데이터는 기존 외래키의 cascade 정책으로 함께 정리된다.
- 삭제 성공 후 로컬 인증 세션을 종료하고 첫 로그인 화면으로 이동하도록 연결했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 2건만 남아 있다.

## [2026-07-23] 관리자별 기본 소속 반 이름 지원

- 모든 학생이 자동 소속되는 `is_default` 반을 기본 소속 반으로 정의하고, 새 학원에서는 이름을 `기본반`으로 생성하도록 변경했다.
- 기존 학원의 기본 소속 반 이름은 강제로 변경하지 않아 정원T 관리자는 `정규반`을 그대로 사용할 수 있게 했다.
- 반 관리에서 기본 소속 반 이름도 수정할 수 있도록 했으며, 랭킹 집계 공식과 이벤트 대상 선택에는 관리자가 정한 실제 반 이름을 표시한다.
- 연결되지 않은 학생도 가입 시 자신이 속한 학원의 기본 소속 반에 자동 승인 상태로 들어가는 기존 흐름을 확인했다.
- 상태: TypeScript 검사와 변경 파일 형식 검사를 통과했다.
- 다음 할 일: `20260723_02_dedupe_historical_attendance.sql` 적용 후 실제 학생별 전체·특강반 합계를 대조한다.

## [2026-07-23] 랭킹 노출 설정에 집계 공식 안내 추가

- 관리자 랭킹 노출 설정 하단에 전체·정규반·특강반별 랭킹 집계 공식을 안내하는 블록을 추가했다.
- 전체는 `하루 1회 출석 + 모든 특강반 과제 + 모든 칭찬`, 정규반은 `하루 1회 출석 + 모든 칭찬`, 특강반은 `하루 1회 출석 + 해당 특강반 과제 + 모든 칭찬`으로 표시한다.
- 출석은 한국 날짜 기준 하루 첫 활성 기록 한 건만 집계하고 취소·롤백 기록은 제외한다는 기준을 함께 명시했다.
- 상태: TypeScript 검사와 변경 파일 형식 검사를 통과했다.
## [2026-07-26] 학생 목록 모바일 테이블 및 소속반 필터 개선

- 모바일 학생 목록에서 가로 스크롤 없이 이름·생년월일·소속반·스티커·처리 항목이 한 화면에 들어오도록 열 너비와 여백, 버튼 크기를 조정했다.
- 모바일 생년월일은 `YY.MM.DD` 형식으로 간결하게 표시하고 넓은 화면에서는 기존 전체 날짜를 유지했다.
- 여러 소속반은 한 줄에 하나씩 세로로 표시하며, 긴 반 이름은 테이블 밖으로 넘치지 않도록 처리했다.
- 학생 목록 상단에 `전체` 및 소속반별 필터 태그를 추가하고, 선택한 반의 학생만 바로 확인할 수 있게 했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.
## [2026-07-28] 출결·과제 소급 신청 및 관리자 수정 승인

- 학생 스티커 화면에서 선택한 오늘 또는 과거 날짜에 출석·과제를 뒤늦게 신청할 수 있도록 날짜 선택과 실제 저장 날짜를 연결했다.
- 반려된 출석·과제 기록은 같은 날짜에 내용을 다시 선택해 재신청할 수 있으며, 승인 또는 대기 중인 기록은 기존 하루 1회 제한을 유지한다.
- 관리자 승인함을 카드형 화면으로 개편하고 출석 기준·과제 완료율·지급 스티커 수를 수정한 뒤 승인할 수 있도록 했다.
- 승인 권한이 있는 관리자와 조교가 학생·날짜·출석 또는 과제 내용을 선택해 직접 등록하고 즉시 스티커를 지급할 수 있도록 했다.
- 과거 날짜 요청을 승인하거나 직접 등록하면 해당 날짜로 스티커 원장을 기록해 일별 합계와 기간 랭킹에도 올바르게 반영되도록 했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.
## [2026-07-28] 스티커 로그 지급 수 수정 및 변경 이력 보존

- 관리자 스티커 로그의 정상 지급 건에 `지급 수정` 버튼을 추가하고 변경할 수량과 수정 사유를 입력할 수 있도록 했다.
- 지급 수 수정 시 기존 원장을 덮어쓰지 않고 `기존 수량 → 변경 수량`과 사유를 남긴 취소 이력으로 보존한 뒤, 같은 지급일의 새 정상 원장을 생성한다.
- 출석·과제·칭찬 원본 신청의 지급 수도 함께 갱신해 학생 화면의 승인 결과와 스티커 원장이 일치하도록 했다.
- 수정 지급 건에는 별도 표시를 추가했으며 기존 롤백도 서버 응답을 확인한 뒤 최신 합계와 랭킹을 즉시 다시 불러오도록 변경했다.
- 관리자 원장은 모두 수정할 수 있고 조교는 `스티커 로그 감사` 권한이 켜진 경우에만 서버에서 수정·롤백할 수 있도록 제한했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.
## [2026-07-28] 승인 반려 이력 스티커 로그 영구 보존

- 스티커 로그에 출석·과제·칭찬 반려 요청을 `반려됨`, `미지급` 상태로 함께 표시하도록 변경했다.
- 반려 시 0장짜리 비집계 감사 원장을 생성해 학생이 같은 요청을 재신청하거나 이후 승인받아도 과거 반려 이력이 사라지지 않도록 했다.
- 반려 당시 출석 기준, 과제 완료율·신청 수량 또는 칭찬 사유를 감사 원장에 함께 기록한다.
- 상태 필터에 `반려됨`을 추가하고 학생·유형 필터 및 최신순 정렬에 반려 이력도 동일하게 포함했다.
- 기존 반려 요청도 영구 감사 원장으로 보정하는 `20260728_01_backfill_rejection_audit_logs.sql` 마이그레이션을 추가했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.
## [2026-07-28] 관리자 학생 특강반 직접 추가

- 학생 목록의 소속반 칸에서 모든 학생에게 자동 적용되는 기본 정규반 표기를 숨기고 특강반만 표시하도록 변경했다.
- 각 학생의 소속반 칸에 `+` 버튼을 추가하고 아직 소속되지 않은 활성 특강반을 여러 개 선택할 수 있는 모바일 친화적 팝업을 구현했다.
- 선생님이 추가한 반은 별도 학생 승인 요청 없이 즉시 승인된 소속반으로 저장되며, 기존 대기·반려 신청이 있으면 승인 상태로 갱신된다.
- 기본 정규반은 추가·해지 대상에서 제외하고, 기존 특강반별 해지 버튼과 상단 특강반 필터는 유지했다.
- 학생 관리 권한 검사를 기존 API와 동일하게 적용하고 다른 학원 반 또는 비활성 반 추가를 서버에서 차단했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.
## [2026-07-28] 승인함·스티커 로그를 스티커 관리로 통합

- 관리자 사이드바의 `승인함`과 `스티커 로그 · 감사` 메뉴를 `스티커 관리` 하나로 통합했다.
- 스티커 관리 화면 내부를 `승인 대기`와 `처리 이력` 탭으로 구분하고 승인 대기 수를 배지로 표시한다.
- 승인 권한과 스티커 로그 감사 권한은 기존처럼 독립적으로 유지하며, 조교에게 허용된 탭만 노출한다.
- 기존 `/admin/approvals`, `/admin/logs` 주소는 각각 새 통합 화면의 해당 탭으로 자동 연결해 알림·즐겨찾기 호환성을 유지했다.
- 상단 알림의 칭찬 승인 대기 링크도 새 승인 대기 탭으로 변경했다.
- 원격 `main`과 작업 시작 기준이 동일한 것을 확인해 외부 변경과의 병합 충돌 가능성이 없었고, 미푸시 반려 로그·학생 특강반 추가 변경과 함께 전체 빌드했다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.
## [2026-07-28] 학생 마이 프로필 이름 수정 명확화

- 학생 마이페이지의 모호한 연필 아이콘을 `이름 수정` 텍스트 버튼으로 변경해 이름 편집 기능을 쉽게 찾을 수 있도록 했다.
- 편집 상태에서 이름 입력 안내와 `프로필 저장` 버튼을 명확히 표시하고, 수정 이름이 홈 화면과 랭킹에 동일하게 반영된다는 설명을 추가했다.
- 빈 이름은 저장하지 않고 오류를 안내하며, 학생 이름을 30자 이내로 제한하는 클라이언트·서버 검증을 적용했다.
- 서버 저장 시 현재 학생과 학원 범위를 함께 확인하고 저장된 프로필 값을 응답하도록 보강했다.
- 기존 전역 학생 프로필 갱신 동작을 통해 저장 직후 홈 프로필, 랭킹, 이벤트 순위의 학생 이름이 함께 변경되는 것을 확인했다.
- 원격 `main`과 작업 시작 기준이 동일해 충돌 가능성이 없었으며 TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.
## [2026-07-28] 초대 링크 역할별 회원가입 화면 직행

- 학생 초대 링크를 `/join/{초대코드}`, 선생님 초대 링크를 `/join/teacher/{초대코드}` 형식으로 명확히 분리했다.
- 전용 초대 경로는 클라이언트 로딩을 거치지 않고 서버에서 각각 `type=student`, `type=teacher`가 포함된 회원가입 화면으로 즉시 이동한다.
- 초대 회원가입 화면 제목을 `학생 회원가입` 또는 `선생님 회원가입`으로 표시하고 다른 가입 유형을 선택하는 버튼을 숨겨 역할 혼동을 방지했다.
- 조직 관리에서 표시·복사하는 전체 초대 링크도 새 역할별 전용 주소로 변경했다.
- 기존 `/signup?invite=...`, `/signup?type=teacher&invite=...` 링크는 계속 호환되며 초대 정보 조회 결과가 최종 역할을 고정한다.
- 상태: TypeScript 검사, 변경 파일 형식 검사, 프로덕션 빌드를 통과했다. 기존 이미지 최적화 경고 3건만 남아 있다.
## 2026-07-28 (학생 마이페이지 푸터 및 정책 페이지)

- 학생 앱 마이페이지 하단에 StickerUp 서비스 소개, 제작자 표기, 고객지원 문의하기, 이용약관, 개인정보 처리방침, 저작권 정보를 포함한 모바일 푸터를 추가했다.
- 문의하기는 후속 작업에서 앱 내부 문의 접수 양식으로 확장했다.
- `/terms`, `/privacy` 읽기 전용 페이지를 추가하고 개인정보 처리 책임자와 저작권자를 Jeongwon Kim으로 표기했다.
- 상태: `npm.cmd run typecheck`와 `npm.cmd run build` 통과.
- 다음 할 일: 앱 내부 문의 게시판 저장 구조를 연결한다.
## 2026-07-28 (개발자 푸터 관리)

- 개발자 상품 관리 화면에 `푸터 관리` 탭을 추가해 서비스 소개, 제작자, 고객지원 문구와 이메일, 약관 링크, 저작권 문구를 수정할 수 있게 했다.
- 전역 푸터 설정 API와 `app_content_settings` 테이블 마이그레이션을 추가했다. 푸터 저장은 개발자 계정만 가능하고, 학생 마이페이지 조회는 저장값이 없거나 테이블 적용 전이면 안전하게 기본값을 사용한다.
- 학생 마이페이지가 공통 푸터 설정을 불러와 표시하도록 연결했으며 개발자 화면에 실제 출력 형태의 미리보기를 제공한다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: Supabase에 `20260728_02_app_content_settings.sql`을 적용한 뒤 개발자 화면에서 푸터 문구를 저장한다.

## 2026-07-28 (앱 내부 고객 문의 게시판)

- 학생 마이페이지의 문의하기 버튼을 메일 연결 대신 중앙 문의창으로 변경했다. 성함, 답변받을 연락처, 문의 내용을 필수로 받고 접수 완료 안내를 표시한다.
- 개발자 화면에 `문의 관리` 탭을 추가했다. 문의를 게시판 형태로 펼쳐 보고 전체·접수·확인 중·답변 완료 상태로 필터링하거나 처리 상태를 변경할 수 있다.
...
## 2026-07-28 (학생 푸터 문구 간소화)

- 푸터 제작자 표기 위에 `학원 출석 · 숙제 · 칭찬 스티커 랭킹 앱` 문구를 추가했다.
- 학생 푸터의 고객지원 제목과 안내 문단을 삭제하고 문의하기 버튼만 남겼다. 버튼을 누른 뒤 표시되는 문의 접수 양식 안내는 유지했다.
- 개발자 푸터 관리에서는 제거된 고객지원 문구 입력란 대신 서비스 설명을 수정할 수 있게 하고 미리보기도 학생 화면과 동일하게 맞췄다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: 배포 화면에서 푸터 간격과 문의 버튼 동작을 확인한다.
## 2026-07-28 (로그인 화면 저작권 표기)

- 메인 로그인 화면과 `/login` 로그인 카드 바로 아래에 `Copyright © 2026 Jeongwon Kim. All rights reserved.` 문구를 추가했다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: 모바일 로그인 화면에서 카드와 저작권 문구 간격을 확인한다.
## 2026-07-28 (환경설정 공용 푸터)

- 학생 마이페이지에 있던 푸터와 문의 양식을 `AppFooter` 공용 컴포넌트로 분리했다.
- 학생 앱 환경설정과 관리자 앱 환경설정 하단에도 동일한 로고, 서비스 설명, 제작자, 문의하기, 약관 링크와 저작권 정보를 표시했다.
- 세 화면 모두 개발자 푸터 관리에서 저장한 값을 사용하며 학생과 관리자는 각 환경설정 화면에서 동일한 문의 양식을 접수할 수 있다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: 모바일 학생 환경설정과 데스크톱·태블릿 관리자 설정에서 푸터 폭과 간격을 확인한다.
## 2026-07-28 (관리자 프로필 사진 기능 제거)

- 관리자 환경설정에서 사진 추가, 미리보기, 제거 기능과 모든 관리자 화면에 적용된다는 안내 문구를 삭제했다.
- 관리자 프로필 저장은 이름만 처리하고 `profile_image_url`을 비우도록 변경했다. 이전 클라이언트의 사진 업로드 요청도 API에서 지원 종료 안내와 함께 차단한다.
- 기존 관리자 사진을 제거하는 `20260728_04_disable_teacher_profile_images.sql` 마이그레이션을 추가했다. 학생 프로필 사진 기능은 영향을 받지 않는다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: Supabase에 신규 마이그레이션을 적용하고 관리자 상단바가 이름 이니셜로 표시되는지 확인한다.
## 2026-07-28 (학생 스티커 신청 탭 확대)

- 학생 앱 스티커 화면의 출석·숙제·칭찬 칩 탭을 첨부 시안과 같은 3분할 세그먼트 탭으로 변경했다.
- 세 버튼이 화면 너비를 균등하게 사용하고 최소 48px 높이와 큰 글자를 갖도록 해 모바일 가독성과 터치 영역을 개선했다.
- 선택 탭은 주황색 배경과 검정색 글자로, 비선택 탭은 어두운 배경의 보조 글자로 표시한다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: 실제 모바일 화면에서 세 탭의 너비와 선택 전환을 확인한다.
## 2026-07-28 (로그인 브랜드 제작자 표기)

- 메인 로그인 화면의 `학원 출석 · 숙제 · 칭찬 스티커 랭킹 앱` 문구 바로 아래에 `by. Jeongwon Kim` 제작자 표기를 추가했다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: 모바일 로그인 화면에서 서비스 설명과 제작자 표기의 간격을 확인한다.
## 2026-07-28 (세션 마감 정리)

- 오늘 작업한 초대 링크 역할별 회원가입, 학생·관리자 화면 푸터, 이용약관·개인정보 처리방침, 앱 내부 문의 게시판, 로그인 저작권·제작자 표기, 관리자 프로필 사진 제거, 학생 스티커 신청 탭 확대를 `main`에 반영했다.
- 최종 검증: 마지막 변경 기준 `npm.cmd run typecheck`, `npm.cmd run build` 통과. 기존 공지·배너의 `<img>` 최적화 경고 3건 외 신규 오류는 없다.
- 운영 반영 필요: Supabase에 `20260728_02_app_content_settings.sql`, `20260728_03_support_inquiries.sql`, `20260728_04_disable_teacher_profile_images.sql`을 순서대로 적용한다.
- 배포 후 확인: 개발자 푸터 저장, 학생·관리자 문의 접수/조회, 관리자 이니셜 표시, 모바일 스티커 출석·숙제·칭찬 탭을 점검한다.
- 상태: 오늘 작업 종료.

## 2026-07-30 (신규 관리자 학생 초대·로그인 오류 디버깅)

- 신규 관리자가 만든 최신 학생 초대 행을 운영 Supabase에서 확인했다. 초대 상태는 `active`, 만료일은 없음, 학원·발급 관리자 연결도 정상이며 배포된 초대 미리보기 API에서 학원명과 학생 역할이 정상 반환되는 것을 확인했다.
- 초대 미리보기 API가 실제 만료/무효(404)와 DB·네트워크 조회 실패(500)를 서로 다른 코드와 한국어 안내로 반환하도록 변경했다.
- 학생 회원가입 화면에서 초대 조회 실패를 무조건 만료로 표시하지 않게 하고, 학원 정보를 확인하는 동안 안내 문구와 `초대 정보 다시 확인` 버튼을 추가했다. 초대 가입에서는 학원명을 초대 데이터의 고정값으로 유지한다.
- 배포된 `/login`이 요청 즉시 500을 반환하는 현상을 재현했다. 로그인 경로를 동적 렌더링으로 고정해 Vercel에서 정적 오류 페이지로 잘못 매핑되는 것을 방지했다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과. 프로덕션 모드 로컬 서버에서 `/login?type=student` 200, 최신 초대 API 200 및 학원명 로드를 확인했다.
- 다음 할 일: 변경사항을 배포한 뒤 신규 관리자 초대 링크를 모바일 시크릿 창에서 열어 회원가입과 로그인 화면 이동을 최종 확인한다.

## 2026-07-30 (관리자·선생님·학생 소속 구조 점검)

- DB 관계와 관리자 조회 범위를 확인했다. 학원(`tenant`)마다 원장 관리자 1명과 여러 하위 선생님 계정이 소속되며, 초대 선생님은 `teachers.invited_by`, 초대 학생은 `students.invited_by_teacher_id`로 발급 선생님과 연결된다.
- 다만 관리자 기능의 실제 데이터 접근 범위는 개별 선생님 연결이 아니라 `tenant_id` 기준이다. 같은 학원의 권한 있는 선생님은 특정 선생님이 초대한 학생만이 아니라 학원 전체 학생을 조회·관리한다.
- 상태: 코드 변경 없이 현재 구조 확인 완료.
- 다음 할 일: 선생님별 담당 학생만 분리 관리해야 한다면 학생 조회·승인 API에 담당자 기준 필터와 원장 전체 조회 예외를 별도로 설계한다.

## 2026-07-30 (복수 관리자 학원별 데이터 격리 점검)

- 관리자 가입 시 학원별 고유 `tenant_id`가 생성되고, 학생 초대 가입은 해당 초대 링크의 `tenant_id`를 그대로 사용함을 확인했다.
- 학생 목록·승인·스티커 로그·반·공지·랭킹·이벤트·상품 API가 로그인 관리자의 `tenant_id`로 조회 및 변경 대상을 제한하며, DB RLS 정책도 `current_tenant_id()` 기준으로 격리한다.
- 운영 DB에는 5개 학원 조직이 있고 다른 학원의 선생님을 학생 연결자로 가리키는 교차 연결은 0건이었다.
- 예외로 관리자 없이 학생만 4명 소속된 학원 조직 1개와 계정이 없는 빈 조직 2개가 확인됐다. 초대 링크 없이 학생이 학원명을 직접 입력해 가입할 때 기존 관리자 학원과 정확히 연결되지 않고 별도 조직이 생성될 수 있는 현재 온보딩 구조의 결과다.
- 상태: 초대 링크를 통한 관리자별 학생 분리는 정상이며 코드 변경 없이 점검 완료.
- 다음 할 일: 운영 안정성을 위해 일반 학생 가입에서는 임의 학원 생성·이름 매칭을 중단하고, 관리자 연결 전 학생은 별도 미연결 상태로 보관한 뒤 초대 또는 연결 승인 시에만 관리자 학원 `tenant_id`로 이동시키는 보완을 검토한다.

## 2026-07-30 (관리자 없는 학생의 선생님 연결 흐름 점검)

- 학생 홈의 `선생님 연결하기`는 `student_connection_requests`에 7일 만료 토큰을 만들고 `/connect/student?token=...` 링크를 발급한다.
- 링크를 연 선생님은 로그인 후 학생 관리 화면으로 이동하지만, 학생 관리 API가 이미 선생님과 같은 `tenant_id`인 학생만 조회한다. 관리자 없는 별도 조직의 학생은 목록에 나타나지 않아 현재 화면 흐름으로 승인할 수 없다.
- 직접 승인하는 이전 동적 경로 API는 학생의 `tenant_id`와 `invited_by_teacher_id`를 선생님 조직으로 바꾸고 새 기본반 등록을 추가하지만, 현재 발급 링크에서는 이 API가 호출되지 않는다. 또한 기존 조직의 반 등록 등 종속 데이터 정리 없이 학생 행만 이동하는 문제가 있다.
- 운영 DB 확인 결과 관리자 없는 조직의 학생 4명은 연결 선생님 0명, 대기 연결 요청 8건, 승인 요청 0건이다.
- 상태: 현재 연결 흐름이 동작하지 않는 원인 확인 완료. 코드 수정은 아직 진행하지 않음.
- 다음 할 일: 연결 링크 화면에서 선생님이 직접 승인하도록 단일화하고, 승인 트랜잭션에서 학생 소속 변경·기존 기본반 정리·새 기본반 등록·연결 요청 완료를 원자적으로 처리해야 한다.

## 2026-07-30 (미연결 학생 선생님 연결 팝업 및 승인 복구)

- 연결 선생님이 없는 학생은 홈 진입 시 중앙 `선생님 연결이 필요합니다` 팝업을 자동으로 보며, 팝업에서 연결 링크를 발급하고 복사할 수 있게 했다. 연결 전에는 특강반 선택 팝업이 동시에 뜨지 않도록 우선순위를 정리했다.
- 유효한 대기 연결 링크가 있으면 새 요청을 계속 만들지 않고 기존 링크를 재사용한다.
- 선생님이 연결 링크를 열었을 때 미로그인 상태라면 해당 링크로 돌아오는 선생님 로그인을 제공하고, 로그인 상태라면 그 자리에서 `학생 연결 승인`을 실행하도록 변경했다.
- 승인 시 학생을 선생님의 학원과 연결하고 새 학원의 기본반에 승인 등록한 다음 이전 임시 조직의 반 소속을 제거한다. 동시에 여러 선생님이 승인할 때는 먼저 완료된 한 명만 연결되도록 조건부 갱신을 추가했다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: 배포 후 관리자 없는 학생 계정으로 링크 발급 → 선생님 로그인 → 연결 승인 → 학생 홈 재접속 흐름을 확인한다.

## 2026-07-30 (운영 DB 미연결 학생·연결 링크 확인)

- 운영 DB에서 `invited_by_teacher_id is null`인 학생을 조회한 결과 4명이며 모두 관리자 없는 `더프라임` 임시 조직에 속해 있었다.
- 학생별 최신 연결 요청은 모두 `pending`이고 2026-08-03~05까지 유효했다. 연결 완료된 학생은 0명이다.
- 상태: 학생 이름, 최신 링크 발급일과 만료일을 사용자에게 전달할 수 있도록 확인 완료. DB 데이터 변경은 하지 않음.
- 다음 할 일: 연결 승인 수정사항을 배포한 뒤 각 유효 링크를 선생님 계정으로 열어 승인한다.

## 2026-07-30 (자동 로그인 세션 저장 수정)

- 전역 앱 초기화에서 Supabase 브라우저 클라이언트가 로그인 폼보다 먼저 생성되면, 이후 자동 로그인 체크값을 변경해도 인증 저장소가 최초 선택값으로 고정되는 문제를 수정했다.
- 인증 저장소 어댑터가 매 요청마다 현재 자동 로그인 설정을 확인하도록 변경했다. 자동 로그인 사용 시 `localStorage`, 미사용 시 `sessionStorage`에 세션을 저장한다.
- 설정 변경 시 기존 Supabase 인증 키를 선택한 저장소로 옮기고 반대편 저장소에서는 제거한다. 이전 버전에서 잘못된 저장소에 남은 세션도 최초 조회 때 복구한다.
- Supabase 토큰 자동 갱신을 명시적으로 활성화했다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: 배포 후 자동 로그인 선택 상태로 로그인하고 브라우저 완전 종료·재실행 뒤 로그인 유지 여부를 확인한다.

## 2026-07-30 (운영 연결 링크 500 재확인)

- 운영 환경에서 실제 미연결 학생 링크를 다시 요청했다. `/connect/student?token=...` 화면과 `/api/student-connections/[token]` 요청은 모두 HTTP 200이었다.
- 연결 화면의 `선생님 로그인`이 이동하는 `/login?type=teacher&next=...` 경로는 HTTP 500이며 `x-matched-path: /500`으로 재현됐다.
- 원인은 앞서 완료한 `/login` 동적 렌더링 수정과 연결 화면 직접 승인 수정이 아직 Git push·Vercel 배포되지 않아 운영 사이트가 이전 버전을 사용하기 때문이다.
- 상태: 추가 코드 오류는 발견되지 않았고 배포 대기 상태.
- 다음 할 일: 현재 변경사항을 push하여 배포한 뒤 같은 연결 링크에서 선생님 로그인과 직접 승인을 다시 확인한다.

## 2026-07-30 (스티커 승인 수량·이벤트 지급 인원 입력 개선)

- 승인 대기 카드의 기준·지급수 입력과 처리 버튼을 분리했다. 모바일에서도 항상 보이는 전체 너비의 `수정 수량으로 승인·지급` 버튼을 추가하고 처리 중 상태를 표시한다.
- 승인 대기 및 선생님 직접 등록의 지급 스티커 입력은 편집 중 전체 내용을 지워 빈 값으로 둘 수 있게 했다. 승인·등록 시점에만 0~100 범위를 검증한다.
- 처리 이력의 지급 수정 수량도 빈 값 편집을 허용하고 저장 시 검증하도록 변경했다.
- 이벤트 생성·수정의 지급 인원 및 비율 입력에서 빈 값을 즉시 `1`로 되돌리던 처리를 제거했다. 기존 `3`을 모두 지운 뒤 `5`를 정상 입력할 수 있으며 저장 시 인원은 1명 이상, 비율은 1~100으로 검증한다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과.
- 다음 할 일: 배포 후 모바일 승인 대기 카드와 이벤트 수정 팝업에서 전체 선택·삭제·재입력·저장을 확인한다.

## 2026-07-30 (미연결 학생 이름 재확인)

- 운영 DB에서 연결 선생님 값이 없는 학생을 다시 조회했으며 현재도 총 4명이다.
- 확인된 이름: 바보의령, 귤똥별똥별, 김한양, 우주.
- 상태: 조회 완료. DB 변경 없음.
- 다음 할 일: 연결 승인 기능 배포 후 해당 학생들의 연결값이 정상 갱신되는지 확인한다.

## 2026-07-30 (우주 학생을 더 프라임 원장과 연결)

- 운영 DB에서 우주 학생을 `더 프라임` 학원으로 이동하고 원장 김정원(정원T)을 연결 선생님으로 지정했다.
- 더 프라임의 기본반 `정규반`에 승인 상태로 등록하고 이전 임시 조직의 반 소속을 제거했다.
- 우주 학생의 최신 대기 연결 요청 1건을 승인 완료로 변경했으며 남은 대기 요청은 없다.
- 사후 확인: 원장 연결 정상, 학원 이동 정상, 정규반 승인 등록 정상, 이전 조직 반 등록 0건. 전체 미연결 학생은 4명에서 3명으로 감소했다.
- 상태: 운영 DB 반영 완료.
- 다음 할 일: 우주 학생이 앱을 새로고침하거나 재로그인한 뒤 더 프라임 정보와 정규반이 표시되는지 확인한다.

## 2026-07-30 (미연결 학생 전용 대기 상태 및 앱 차단)

- 초대 링크 없이 가입한 학생은 입력한 학원명으로 기존 조직을 검색하거나 합류하지 않도록 변경했다. 학생마다 독립된 `선생님 연결 대기` 조직을 만들고 선생님·학원·기본반 연결값은 생성하지 않는다.
- 일반 학생 회원가입에서 학원 이름 입력란을 제거하고, 가입 후 선생님 승인을 받으면 학원과 기본반이 자동 설정된다는 안내를 표시한다. 초대 링크 가입은 초대 학원명이 고정되는 기존 흐름을 유지한다.
- 학생 앱 레이아웃에 연결 게이트를 추가했다. 미연결 학생은 상단바·하단 메뉴·홈·스티커·랭킹·마이페이지 대신 연결 링크 발급 전용 화면만 볼 수 있다.
- `/api/app-state`, `/api/student/home`, `/api/student/actions`도 미연결 학생의 일반 데이터 조회·행동 요청을 403으로 차단해 주소 직접 호출로 우회할 수 없게 했다.
- 선생님 연결 승인 시 실제 학원으로 이동하고 기본반에 등록한 뒤, 이전 학생 전용 대기 조직이 비면 자동 삭제한다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과. DB 마이그레이션 없음.
- 다음 할 일: 배포 후 초대 없는 신규 학생 가입 → 연결 전용 화면 → 링크 발급 → 선생님 승인 → 정상 학생 앱 진입을 확인한다.

## 2026-07-30 (기존 미연결 학생 3명 삭제)

- 운영 DB에서 선생님 연결값이 없는 바보의령, 귤똥별똥별, 김한양 계정을 고유 ID와 임시 조직 소속으로 재검증한 뒤 삭제했다.
- Supabase 인증 계정을 삭제해 학생 프로필, 연결 요청 7건과 기본반 등록 3건을 연쇄 삭제했다.
- 세 학생이 빠진 뒤 관리자·선생님·학생이 모두 없는 기존 임시 `더프라임` 조직도 삭제했다.
- 사후 확인: 대상 학생 행 0건, 연결 요청 0건, 반 등록 0건, 전체 미연결 학생 0명.
- 상태: 운영 DB 삭제 완료. 삭제된 계정은 복구할 수 없음.
- 다음 할 일: 없음.

## 2026-07-30 (운영 Supabase 마이그레이션 상태 점검)

- 현재 미연결 학생 전용 대기 상태 변경은 기존 `tenants`, `students`, `enrollments`, `student_connection_requests` 구조만 사용하므로 신규 SQL 마이그레이션이 필요하지 않음을 확인했다.
- 운영 Supabase에서 선생님 권한·프로필, 역할별 초대, 연결 요청, 상품 카탈로그·추천 상품·배너, 랭킹 커스텀 기간, 공지 이미지, 이벤트 제목·설명, 학생 생년월일, 출석·과제 승인, 경품 선호, 앱 푸터 설정과 고객 문의 테이블 및 관련 컬럼을 직접 조회했다.
- `app_content_settings`, `support_inquiries`가 존재하고 관리자 프로필 이미지도 모두 null로 정리돼 있어 20260728의 최근 마이그레이션이 운영 DB에 반영된 상태다.
- 상태: Supabase SQL Editor에서 추가로 실행할 작업 없음.
- 다음 할 일: 코드 변경사항 Git push 및 Vercel 배포 후 기능 흐름만 확인한다.
## 2026-07-30 (학원별 스티커 지급 정책 확장 가능성 점검)

- 한 일: 관리자 스티커 정책 화면, 학생 출석·과제 신청 API, 관리자 승인 API, 전역 상태와 DB 스키마를 대조해 학원별 지급 기준 편집 가능 여부와 충돌 지점을 점검했다.
- 상태: 현재 정책 화면은 조회 전용이며, 출석·과제 지급 기준은 클라이언트 기본값과 서버 코드의 공통 상수로 고정되어 있다. 학원별 정책 저장 테이블과 조회·저장 API는 없고, 원장별 설정이 실제 신청·승인 수량에 반영되지 않는다.
- 다음 할 일: 구현 시 `tenant_id` 단위 정책/버전 테이블, 원장 전용 편집 권한, 신청 당시 정책 스냅샷, 비활성화 방식, 서버 검증 및 학생 UI 연동을 함께 추가한다. 임의 정책 유형(예: 성적)을 지원하려면 요청·승인·원장 직접 지급·감사 로그·랭킹 범위를 포괄하는 범용 정책 구조와 DB 마이그레이션이 추가로 필요하다.
## 2026-07-31 (공통 정책 템플릿 + 학원별 정책 인스턴스 구조 검토)

- 한 일: 기존 출석·과제·칭찬 기록 구조를 유지하면서 더 프라임의 현재 기준을 학원 정책으로 이전하고, 이후 성적·발표·상담 등의 자유 정책을 추가할 수 있는 구조를 검토했다.
- 상태: 공통 정책 템플릿과 `tenant_id` 단위 학원 정책 사본을 분리하는 혼합 구조가 적합하다. 학원이 템플릿을 선택하면 독립 사본과 버전을 만들고, 신청 시 정책 버전·항목·표시명·예정 지급량을 스냅샷으로 저장하면 기존 승인대기 및 과거 원장 기록을 재계산하지 않고 확장할 수 있다.
- 다음 할 일: 구현 시 템플릿/학원 정책/정책 버전/범용 신청 테이블과 RLS를 추가하고, 더 프라임의 현재 출석·과제·칭찬 기준을 최초 학원 정책으로 이관한다. 기존 승인대기 건은 `legacy` 정책 스냅샷으로 보정하며, 정책 삭제 대신 비활성화하고 변경은 새 버전부터 적용한다.
## 2026-07-31 (정규반 과제 스티커 신청 가능 여부 점검)

- 한 일: 학생 과제 화면, 학생 신청 API, 관리자 직접 등록 화면/API, 과제 중복 제한과 랭킹 집계를 확인했다.
- 상태: `homework_submissions`는 모든 반의 `class_id`를 저장할 수 있고 학생·반·날짜별 1회 제한도 이미 있어 DB상 정규반 과제를 지원할 수 있다. 현재는 학생 과제 반 목록과 관리자 직접 등록 반 목록에서 `is_default` 반을 제외하고, 두 API도 기본반이면 거절하도록 명시적으로 차단되어 있다.
- 영향: 해당 차단만 일관되게 해제하면 정규반 과제 신청·승인·로그 저장이 가능하다. 전체 랭킹에는 해당 과제가 포함되고, 정규반 랭킹에서는 정규반 과제만 포함되며, 특강반 랭킹에서는 기존대로 해당 특강반 과제만 포함된다.
- 다음 할 일: 구현 요청 시 학생/관리자 반 선택 UI와 서버 검증을 함께 변경하고, 기본반 승인 등록이 없는 예외 계정 처리 및 전체·정규반·특강반 랭킹 회귀 테스트를 수행한다.
## 2026-07-31 (정규반 과제 신청 및 상품 제휴처 필터)

- 한 일: 학생 과제 반 선택과 관리자 과제 직접 등록에서 승인된 정규반을 허용하고, 학생/관리자 API의 특강반 전용 제한을 승인된 소속반 기준으로 변경했다.
- 한 일: 관리자 이벤트·상품 관리의 추천상품/경품 리스트 필터에 제휴처 조건을 카테고리보다 위에 추가했다. 개발자 상품 리스트에도 동일한 제휴처·카테고리 필터와 카드별 제휴처 표시를 적용했다.
- 구조: 제휴처 판별을 `lib/product-affiliate.ts`로 공통화해 쿠팡, TEMU, 기타 제휴, 직접 등록 표시와 필터 결과가 두 화면에서 동일하게 동작하도록 했다.
- 상태: `npm.cmd run typecheck`, `npm.cmd run build` 통과. DB 마이그레이션은 필요 없다.
- 다음 할 일: 배포 후 학생 계정에서 정규반과 복수 특강반이 과제 선택 목록에 함께 표시되는지, 반별 같은 날짜 1회 제한과 관리자 승인 후 각 반 랭킹 반영을 확인한다.
## 2026-07-31 (관리자 상단 승인 대기 표시 통합)

- 한 일: 관리자 상단의 `승인 대기 n건` 텍스트를 삭제하고 종 모양 알림 버튼의 배지 안에 승인 대기 건수를 숫자로 표시하도록 통합했다.
- 상태: 알림 팝업과 이동 링크는 기존대로 유지하며, 100건 이상은 `99+`로 표시하고 스크린리더용 알림 설명에도 실제 대기 건수를 포함했다.
- 다음 할 일: 배포 후 모바일 상단에서 배지 숫자와 관리자 이름, 설정 아이콘이 겹치지 않는지 확인한다.
## 2026-07-31 (관리자 칭찬 스티커 직접 등록)

- 한 일: 관리자 스티커 관리의 `직접 등록` 유형에 칭찬을 추가했다. 학생·날짜·칭찬 사유·지급 수량을 입력해 승인 절차 없이 바로 지급할 수 있다.
- 처리: 직접 칭찬 등록 시 승인 완료 상태의 `praise_requests`와 활성 `sticker_ledger`를 함께 생성해 기존 칭찬 승인·랭킹·감사·롤백 구조와 동일하게 기록한다. 같은 학생의 같은 날짜에 반려되지 않은 칭찬 신청 또는 지급 기록이 있으면 중복 등록을 차단한다.
- 상태: 기존 승인 대기 칭찬 건은 지급 수량을 수정한 뒤 승인할 수 있는 기능을 유지한다. `npm.cmd run typecheck`, `npm.cmd run build` 통과. DB 마이그레이션은 필요 없다.
- 다음 할 일: 배포 후 칭찬 직접 등록, 중복 날짜 차단, 스티커 로그 표시와 로그 화면 지급 수량 수정까지 확인한다.
