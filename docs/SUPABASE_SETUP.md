...
- Email signup creates its confirmation return URL from the current browser origin first (for example, `https://stickerup.vercel.app/login`). `NEXT_PUBLIC_SITE_URL` and `NEXT_PUBLIC_APP_URL` are only fallbacks when a browser origin is unavailable. If no valid origin can be created, the app omits `emailRedirectTo` and Supabase uses the configured Site URL instead.
