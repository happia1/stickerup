...
export async function POST(request: Request) {
  ...
  if (action.type === "ADD_CLASS") {
    ...
    const campaign = await db.from("reward_campaigns").insert({ tenant_id: tenantId, class_id: action.classId, period_start: action.periodStart, period_end: action.periodEnd, target_distribution: { type: action.distributionType, value: action.distributionValue }, status: "active" }).select("id").single();
    ...
    const result = await db.from("reward_campaigns").update({ period_start: action.periodStart, period_end: action.periodEnd, target_distribution: { type: action.distributionType, value: action.distributionValue } }).eq("id", action.campaignId).eq("tenant_id", tenantId);
    if (result.error) return NextResponse.json({ error: result.error.message }, { status: 400 });
