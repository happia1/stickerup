...
export async function POST(request: Request) { const context=await getContext(request); if("error" in context)return context.error; const body=await request.json(); if(!body.title?.trim())return NextResponse.json({error:"상품명이 필요합니다."},{status:400}); const result=await context.db.from("product_catalog").insert({tenant_id:context.teacher.tenant_id,title:body.title.trim(),price_label:body.priceLabel?.trim()||null,category:body.category?.trim()||null,image_url:body.imageUrl??null,purchase_url:body.purchaseUrl??null,description:body.description??null}).select("*").single(); if(result.error)return NextResponse.json({error:result.error.message},{status:400}); return NextResponse.json({product:result.data}); }
export async function PATCH(request: Request) { const context=await getContext(request); if("error" in context)return context.error; const body=await request.json(); if(!body.productId||!body.title?.trim())return NextResponse.json({error:"상품 정보를 확인해주세요."},{status:400}); const result=await context.db.from("product_catalog").update({title:body.title.trim(),price_label:body.priceLabel?.trim()||null,category:body.category?.trim()||null,image_url:body.imageUrl??null,purchase_url:body.purchaseUrl??null,description:body.description??null,updated_at:new Date().toISOString()}).eq("id",body.productId).eq("tenant_id",context.teacher.tenant_id).select("*").single(); if(result.error)return NextResponse.json({error:result.error.message},{status:400}); return NextResponse.json({product:result.data}); }
