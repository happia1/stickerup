...
export async function PATCH(request: Request) {
  ...
  const body = await request.json() as { type?: "attendance" | "homework" | "praise"; requestId?: string; action?: "approve" | "reject"; count?: number };
  ...
  const count = body.type === "praise" ? Math.max(1, body.count ?? 2) : row.data.sticker_count;
