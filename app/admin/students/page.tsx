...
export default function AdminStudentsPage() {
  ...
  const classFilters = Array.from(new Map(students.flatMap((student) => student.classMemberships).map((membership) => [membership.classId, membership])).values());
        ...
        <table className="w-full table-fixed text-micro sm:text-body">
          ...
          <tbody>
            ...
            {visibleStudents.map((student) => <Fragment key={student.id}><tr id={`student-${student.id}`} className={`border-b border-border ${student.connectionStatus === "pending" ? "bg-state-warningBg/40" : ""} ${focusedStudentId === student.id ? "outline outline-2 outline-brand-amber outline-offset-[-2px]" : ""}`}>
              ...
              <td className="whitespace-nowrap p-1.5 sm:p-2.5"><span className="sm:hidden">{compactBirthDate(student.birthDate)}</span><span className="hidden sm:inline">{student.birthDate ?? "-"}</span></td><td className="p-1.5 sm:p-2.5"><div className="flex flex-col items-start gap-1">{student.classMemberships.length ? student.classMemberships.map((membership)=><span key={membership.classId} title={membership.className} className="flex w-full min-w-0 items-center gap-1 rounded-full bg-surface-raised px-1.5 py-1 text-micro sm:px-2 sm:text-caption"><span className="min-w-0 truncate">{membership.className}</span>{!membership.isDefault&&<button type="button" aria-label={`${membership.className} 소속 해지`} className="shrink-0 text-state-danger" onClick={()=>{if(window.confirm(`${student.name} 학생의 ${membership.className} 소속을 해지할까요?`))void updateConnection(student,"remove_class",membership.classId);}}>×</button>}</span>) : "-"}</div></td><td className="p-1 text-center font-semibold sm:p-2.5">{student.totalStickers}</td>
