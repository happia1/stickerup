    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        <table className="w-full min-w-[720px] table-fixed text-body">
          <thead><tr className="text-caption text-text-secondary text-left border-b border-border"><th className="w-40 p-2.5">이름</th><th className="w-28 p-2.5">생년월일</th><th className="p-2.5">소속 반</th><th className="w-16 p-2.5">스티커</th><th className="w-48 p-2.5">처리</th></tr></thead>
          <tbody>
            {loading && <tr><td colSpan={5} className="p-5 text-center text-text-secondary">불러오는 중...</td></tr>}
            {!loading && !students.length && <tr><td colSpan={5} className="p-5 text-center text-text-secondary">등록된 학생이 없습니다.</td></tr>}
            {students.map((student) => <Fragment key={student.id}><tr id={`student-${student.id}`} className={`border-b border-border ${student.connectionStatus === "pending" ? "bg-state-warningBg/40" : ""} ${focusedStudentId === student.id ? "outline outline-2 outline-brand-amber outline-offset-[-2px]" : ""}`}>
              <td className="p-2.5 font-semibold"><button type="button" onClick={()=>setExpandedStudentId(current=>current===student.id?null:student.id)} className="text-left">{student.name}</button>{isBirthday(student.birthDate)&&<span className="ml-2 rounded-full bg-brand-amber px-2 py-0.5 text-caption text-surface-page">🎂 오늘 생일</span>}</td>
              <td className="whitespace-nowrap p-2.5">{student.birthDate ?? "-"}</td><td className="p-2.5"><div className="flex flex-nowrap gap-1 overflow-x-auto">{student.classMemberships.length ? student.classMemberships.map((membership)=><span key={membership.classId} className="inline-flex shrink-0 items-center gap-1 whitespace-nowrap rounded-full bg-surface-raised px-2 py-1 text-caption">{membership.className}{!membership.isDefault&&<button type="button" aria-label={`${membership.className} 소속 해지`} className="text-state-danger" onClick={()=>{if(window.confirm(`${student.name} 학생의 ${membership.className} 소속을 해지할까요?`))void updateConnection(student,"remove_class",membership.classId);}}>×</button>}</span>) : "-"}</div></td><td className="p-2.5 text-center">{student.totalStickers}</td>
              <td className="p-2.5"><div className="flex flex-wrap items-center gap-1.5">{student.connectionStatus === "connected" ? <button disabled={processingId === student.id} className="rounded-lg border border-state-danger px-2.5 py-1 text-caption text-state-danger disabled:opacity-50" onClick={() => updateConnection(student, "disconnect")}>연결 해지</button> : student.connectionStatus === "pending" ? <><span className="text-caption font-semibold text-brand-amber">연결중</span><button disabled={processingId === student.id} className="rounded-lg border border-state-success px-2.5 py-1 text-caption text-state-success disabled:opacity-50" onClick={() => updateConnection(student, "approve")}>승인</button><button disabled={processingId === student.id} className="rounded-lg border border-border px-2.5 py-1 text-caption text-text-secondary disabled:opacity-50" onClick={() => updateConnection(student, "revoke_pending")}>해지</button></> : <span className="text-caption text-text-muted">미연결</span>}{canDeleteStudents && <button disabled={processingId === student.id} className="rounded-lg px-2.5 py-1 text-caption text-state-danger disabled:opacity-50" onClick={() => updateConnection(student, "delete")}>삭제</button>}</div></td>
