...
import { saveRewardCampaign } from "@/lib/reward-campaign-client";
...
function parseDistributionValue(rawValue: string, type: "count" | "ratio"): number | null {
  if (rawValue.trim() === "") return null;
  const value = Number(rawValue);
  if (!Number.isFinite(value) || value < 1) return null;
  if (type === "ratio" && value > 100) return null;
  return type === "count" ? Math.floor(value) : value / 100;
}

function EventSettings({ campaign }: { campaign: RewardCampaign }) {
  const state = useAppState();
  const distribution = campaign.target_distribution.type === "count"
    ? `상위 ${campaign.target_distribution.value}명`
    : `상위 ${Math.round(campaign.target_distribution.value * 100)}%`;

  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <div className="sm:col-span-2"><p className="text-caption text-text-muted">이벤트명</p><p className="mt-1 text-body font-semibold">{campaignTitle(campaign, state)}</p></div>
      <div className="sm:col-span-2"><p className="text-caption text-text-muted">설명</p><p className="mt-1 whitespace-pre-wrap text-body text-text-secondary">{campaign.description?.trim() || "설명이 없습니다."}</p></div>
      <div><p className="text-caption text-text-muted">대상 반</p><p className="mt-1 text-body font-semibold">{scopeName(campaign, state)}</p></div>
      <div><p className="text-caption text-text-muted">순위 기준</p><p className="mt-1 text-body font-semibold">{distribution}</p></div>
      <div className="sm:col-span-2"><p className="text-caption text-text-muted">이벤트 기간</p><p className="mt-1 text-body font-semibold">{campaign.period_start} ~ {campaign.period_end}</p></div>
    </div>
  );
}
...
function EditForm({ campaign, close }: { campaign: RewardCampaign; close: () => void }) {
  ...
  const [distValue,setDistValue]=useState(String(campaign.target_distribution.type === "count" ? campaign.target_distribution.value : Math.round(campaign.target_distribution.value * 100)));
  return <div className="mt-3 rounded-xl bg-surface-card p-4"><div className="grid gap-3 sm:grid-cols-2"><label className="text-caption text-text-secondary sm:col-span-2">이벤트명<input className="mt-1 w-full rounded-lg border border-border px-2.5 py-2 text-body" value={title} onChange={event=>setTitle(event.target.value)}/></label><label className="text-caption text-text-secondary sm:col-span-2">설명<textarea className="mt-1 min-h-20 w-full rounded-lg border border-border px-2.5 py-2 text-body" value={description} onChange={event=>setDescription(event.target.value)}/></label><label className="text-caption text-text-secondary">시작일<input type="date" className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={start} onChange={event=>setStart(event.target.value)}/></label><label className="text-caption text-text-secondary">종료일<input type="date" className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={end} onChange={event=>setEnd(event.target.value)}/></label><label className="text-caption text-text-secondary">순위 기준<select className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={distType} onChange={event=>setDistType(event.target.value as "count"|"ratio")}><option value="count">상위 인원</option><option value="ratio">상위 비율</option></select></label><label className="text-caption text-text-secondary">{distType==="count"?"인원":"비율(%)"}<input type="number" min="1" max={distType==="ratio"?100:undefined} className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={distValue} onChange={event=>setDistValue(event.target.value)}/></label></div><div className="mt-4 flex gap-2"><Button onClick={()=>{const parsedDistribution=parseDistributionValue(distValue,distType);if(!title.trim()||!start||!end||start>end)return toast("이벤트명과 기간을 확인해 주세요.");if(parsedDistribution===null)return toast(distType==="count"?"지급 인원을 1명 이상 입력해 주세요.":"지급 비율을 1~100 사이로 입력해 주세요.");const action={type:"UPDATE_REWARD_CAMPAIGN" as const,campaignId:campaign.id,title:title.trim(),description:description.trim(),distributionType:distType,distributionValue:parsedDistribution,periodStart:start,periodEnd:end};const rollback={type:"UPDATE_REWARD_CAMPAIGN" as const,campaignId:campaign.id,title:campaign.title??"",description:campaign.description??"",distributionType:campaign.target_distribution.type,distributionValue:campaign.target_distribution.value,periodStart:campaign.period_start,periodEnd:campaign.period_end};dispatch(action);close();toast("이벤트 수정 내용을 반영했어요.");void saveRewardCampaign(action).catch(error=>{dispatch(rollback);toast(error instanceof Error?`${error.message} 기존 값으로 되돌렸습니다.`:"저장에 실패해 기존 값으로 되돌렸습니다.");});}}>저장</Button><Button variant="secondary" onClick={close}>취소</Button></div></div>;
}
...
export default function AdminRewardsPage() {
  ...
  return <div>
    ...
    {createOpen&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4" onClick={()=>setCreateOpen(false)}><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-card bg-surface-page p-5" onClick={event=>event.stopPropagation()}><h3 className="mb-4 text-subtitle">이벤트 생성</h3><div className="grid gap-3 sm:grid-cols-2"><label className="text-caption text-text-secondary sm:col-span-2">이벤트명<input className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={name} onChange={event=>setName(event.target.value)}/></label><label className="text-caption text-text-secondary sm:col-span-2">설명<textarea className="mt-1 min-h-20 w-full rounded-lg border border-border px-2.5 py-2" value={description} onChange={event=>setDescription(event.target.value)}/></label><label className="text-caption text-text-secondary">대상 반<select className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={scopeId} onChange={event=>setScopeId(event.target.value)}><option value="__all__">전체</option>{regularClass&&<option value="__default__">{regularClass.name}</option>}{specialClasses.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-caption text-text-secondary">순위 기준<select className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={distType} onChange={event=>setDistType(event.target.value as "count"|"ratio")}><option value="count">상위 인원</option><option value="ratio">상위 비율</option></select></label><label className="text-caption text-text-secondary">시작일<input type="date" className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={start} onChange={event=>setStart(event.target.value)}/></label><label className="text-caption text-text-secondary">종료일<input type="date" className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={end} onChange={event=>setEnd(event.target.value)}/></label><label className="text-caption text-text-secondary">{distType==="count"?"인원":"비율(%)"}<input type="number" min="1" max={distType==="ratio"?100:undefined} className="mt-1 w-full rounded-lg border border-border px-2.5 py-2" value={distValue} onChange={event=>setDistValue(event.target.value)}/></label></div><div className="mt-5 flex gap-2"><Button onClick={()=>{const parsedDistribution=parseDistributionValue(distValue,distType);const classId=scopeId==="__all__"?null:scopeId==="__default__"?regularClass?.id??null:scopeId;let periodStart=start;let periodEnd=end;if(!periodStart||!periodEnd){const period=rankingPeriodLabel(state,classId);periodStart=period.start;periodEnd=period.end;}if(!name.trim()||periodStart>periodEnd)return toast("이벤트명과 기간을 확인해 주세요.");if(parsedDistribution===null)return toast(distType==="count"?"지급 인원을 1명 이상 입력해 주세요.":"지급 비율을 1~100 사이로 입력해 주세요.");dispatch({type:"ADD_REWARD_CAMPAIGN",title:name.trim(),description:description.trim(),classId,periodStart,periodEnd,distributionType:distType,distributionValue:parsedDistribution});setName("");setDescription("");setStart("");setEnd("");setDistValue("3");setCreateOpen(false);toast("이벤트를 생성했어요.");}}>생성</Button><Button variant="secondary" onClick={()=>setCreateOpen(false)}>취소</Button></div></div></div>}
  </div>;
}
