  ...
  return <div>
    ...
    <section {...swipeProps}>
      ...
      <div className={view==="card"?"grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5":"space-y-2"}>{paged.map((product,index)=><article key={product.id} className={`${view==="card"?"overflow-hidden":"flex items-center gap-3 p-3"} relative rounded-xl border border-border bg-surface-page`}>
        ...
        <div className={view==="card"?"p-2.5":"flex min-w-0 flex-1 flex-col gap-2 sm:flex-row sm:items-center"}><div className="min-w-0 flex-1"><ProductInfo product={product} compact={view==="list"}/></div><div className={view==="card"?"mt-3 flex items-center gap-1":"flex shrink-0 flex-wrap gap-1.5 sm:justify-end"}>{product.purchase_url&&<a href={product.purchase_url} target="_blank" rel="noreferrer" className="min-w-0 flex-1 whitespace-nowrap rounded-lg border border-border px-1.5 py-2 text-center text-micro font-bold">구매하기</a>}{tab==="recommended"?<button disabled={saved.includes(product.id)} onClick={()=>void addToCart(product.id)} className="min-w-0 flex-1 whitespace-nowrap rounded-lg bg-brand-amber px-1.5 py-2 text-micro font-bold text-surface-page disabled:opacity-50">{saved.includes(product.id)?"담김":"경품 리스트 담기"}</button>:<><button onClick={()=>begin(product)} className="min-w-0 flex-1 whitespace-nowrap rounded-lg border border-border px-1.5 py-2 text-micro">수정</button><button onClick={()=>void remove(product.id)} className="min-w-0 flex-1 whitespace-nowrap rounded-lg px-1.5 py-2 text-micro text-state-danger">삭제</button></>}</div></div>
      </article>)}</div>
