  ...
  return <div>
    ...
    <section {...swipeProps}>
      ...
      <div className={view==="card"?"grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5":"space-y-2"}>{paged.map((product,index)=><article key={product.id} className={`${view==="card"?"overflow-hidden":"flex items-center gap-3 p-3"} relative rounded-xl border border-border bg-surface-page`}>
        {tab==="cart"&&<><span className="pointer-events-none absolute left-2 top-2 z-10 flex h-7 min-w-7 items-center justify-center rounded-full bg-black/70 px-2 text-caption font-bold text-white">{page*PAGE_SIZE+index+1}</span><span className="pointer-events-none absolute right-2 top-2 z-10 flex h-7 items-center justify-center gap-1 rounded-full bg-black/70 px-2 text-caption font-bold text-white">♥ {product.like_count??0}</span></>} {product.purchase_url?<a href={product.purchase_url} target="_blank" rel="noreferrer" aria-label={`${product.title} 상품 페이지 열기`} className="block shrink-0"><img src={product.image_url??"/images/placeholder-product.svg"} onError={event=>{event.currentTarget.onerror=null;event.currentTarget.src="/images/placeholder-product.svg";}} alt={product.title} className={view==="card"?"aspect-square w-full object-cover":"h-20 w-20 shrink-0 rounded-lg object-cover sm:h-24 sm:w-24"}/></a>:<img src={product.image_url??"/images/placeholder-product.svg"} onError={event=>{event.currentTarget.onerror=null;event.currentTarget.src="/images/placeholder-product.svg";}} alt={product.title} className={view==="card"?"aspect-square w-full object-cover":"h-20 w-20 shrink-0 rounded-lg object-cover sm:h-24 sm:w-24"}/>}
        <div className={view==="card"?"p-2.5":"flex min-w-0 flex-1 flex-col gap-2 sm:flex-row sm:items-center"}><div className="min-w-0 flex-1"><ProductInfo product={product} compact={view==="list"}/></div><div className={view==="card"?"mt-3 flex items-center gap-1":"flex shrink-0 flex-wrap gap-1.5 sm:justify-end"}>{tab==="recommended"?<button onClick={()=>void addToCart(product.id)} className="min-w-0 flex-1 whitespace-nowrap rounded-lg bg-brand-amber px-1.5 py-2 text-micro font-bold text-surface-page">경품 리스트 담기</button>:<>{!product.source_marketplace_product_id&&<button onClick={()=>begin(product)} className="min-w-0 flex-1 whitespace-nowrap rounded-lg border border-border px-1.5 py-2 text-micro">수정</button>}<button onClick={()=>void remove(product.id)} className="min-w-0 flex-1 whitespace-nowrap rounded-lg px-1.5 py-2 text-micro text-state-danger">삭제</button></>}</div></div>
      </article>)}</div>
