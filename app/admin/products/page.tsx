...
export default function AdminProductsPage() {
  ...
  return <div>
    <Link href="/admin/rewards" className="text-caption text-text-secondary">이벤트/상품 관리</Link><h2 className="mt-2 text-title">상품 카탈로그</h2><p className="mb-4 mt-1 text-caption text-text-secondary">추천상품을 확인하고 장바구니에 저장해 이벤트 경품으로 사용하세요.</p>
    ...
    <section {...swipeProps}>
      {source.length===0&&!query&&<p className="rounded-xl bg-surface-page p-5 text-caption text-text-secondary">{tab==="cart"?"장바구니가 비어 있습니다.":"조건에 맞는 추천상품이 없습니다."}</p>}
      <div className={view==="card"?"grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5":"space-y-2"}>{paged.map(product=><article key={product.id} className={view==="card"?"overflow-hidden rounded-xl border border-border bg-surface-page":"flex items-center gap-3 rounded-xl border border-border bg-surface-page p-3"}>
        <img src={product.image_url??"/images/placeholder-product.svg"} alt={product.title} className={view==="card"?"aspect-square w-full object-cover":"h-20 w-20 shrink-0 rounded-lg object-cover sm:h-24 sm:w-24"}/>
        <div className={view==="card"?"p-2.5":"flex min-w-0 flex-1 flex-col gap-2 sm:flex-row sm:items-center"}><ProductInfo product={product} compact={view==="list"}/><div className={view==="card"?"mt-3 grid gap-1.5":"flex shrink-0 flex-wrap gap-1.5 sm:justify-end"}>{product.purchase_url&&<a href={product.purchase_url} target="_blank" rel="noreferrer" className="rounded-lg border border-border px-2 py-2 text-center text-micro font-bold">구매하기</a>}{tab==="recommended"?<button disabled={saved.includes(product.id)} onClick={()=>void addToCart(product.id)} className="rounded-lg bg-brand-amber px-2 py-2 text-micro font-bold text-surface-page disabled:opacity-50">{saved.includes(product.id)?"담김":"장바구니 담기"}</button>:<><button onClick={()=>begin(product)} className="rounded-lg border border-border px-2 py-2 text-micro">수정</button><button onClick={()=>void remove(product.id)} className="rounded-lg px-2 py-2 text-micro text-state-danger">삭제</button></>}</div></div>
      </article>)}</div>
