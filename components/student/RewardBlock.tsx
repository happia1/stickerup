...
export function RewardBlock({ data }: { data?: StudentHomeData }) {
  ...
  useEffect(()=>{if(!sheetOpen)return;void getSupabaseBrowserClient()?.auth.getSession().then(async({data})=>{if(!data.session)return;const response=await fetch("/api/student/prize-preferences",{headers:{Authorization:`Bearer ${data.session.access_token}`}});const payload=await response.json();if(response.ok)setPrizes(payload.products);});},[sheetOpen]);
  const toggleLike=async(productId:string)=>{const session=await getSupabaseBrowserClient()?.auth.getSession();const token=session?.data.session?.access_token;if(!token)return;const response=await fetch("/api/student/prize-preferences",{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({productId})});if(!response.ok)return showToast("좋아요를 저장하지 못했어요.");setPrizes(current=>current.map(item=>item.id===productId?{...item,liked_by_me:!item.liked_by_me,like_count:item.like_count+(item.liked_by_me?-1:1)}:item).sort((a,b)=>b.like_count-a.like_count||a.title.localeCompare(b.title,"ko")));};
